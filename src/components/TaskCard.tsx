...
export function TaskCard({ task, onMovePrevious, onMoveNext }: TaskCardProps) {
  ...
  const isLastStatus = currentIndex === statusOrder.length - 1;
  const isFirstStatus = currentIndex === 0;

  const customStatusColors: Record<TaskStatus, string> = {
    todo: "bg-blue-100",
  ...
  return (
    <div
      className={`${customStatusColors[status]} p-4 rounded-md shadow-sm mb-2 flex flex-col`}
    >
